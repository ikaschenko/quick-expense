 ...
 */
export function sendErrorAlertEmail({ message, event, requestId, timestamp, error, path, method, statusCode }) {
  if (alertRecipients.length === 0) return;
  const { subject, html, text } = errorAlertEmail({ message, event, requestId, timestamp, error, path, method, statusCode });
