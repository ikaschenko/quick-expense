...
22. **Home screen metrics cache (`localStorage`)** — dashboard metrics are cached under `qe_metrics_{email}` for instant Home render. Freshness is checked per load via `GET /api/sheet/modifiedtime` (within `drive.file` scope); invalidated at midnight, on sign-out, config clear, and when Drive `modifiedTime` exceeds the stored timestamp. Surgical CRUD mutations rewrite the cache optimistically. Guests and URL-pasted spreadsheets get `modifiedTime: null` and fall back to a full reload (no cache benefit).
