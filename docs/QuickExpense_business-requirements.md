...
- **Transaction date display:** dates in an expanded category's transaction table use the device-local preference selected under Setup → Display preferences. Available formats are Browser default, MM/DD/YYYY, DD/MM/YYYY, and YYYY-MM-DD; Browser default is used until the user selects another format.
